...
Frustum::Frustum(ProjectionConfig::ProjType t) : type(t)
{
	GLuint elem[12 * 2];

	this->orient(vec3(0.0f, 0.0f, 1.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
	if (type == ProjectionConfig::ORTHO) {
...
void Frustum::enclose(const Frustum & other)
{
	...
	if (type == ProjectionConfig::PERSPECTIVE)
	...
	if (other.type == ProjectionConfig::PERSPECTIVE) {
		float dy = (float)other.mNear * tanf(TO_RADIANS(other.fovy) / 2.0f);
...
void Frustum::render() const
{
	if (type == ProjectionConfig::PERSPECTIVE) {
		...
		p[2] = c - u * dx - v * dy;
		p[3] = c + u * dx - v * dy;
		dy = (float)(mFar * tanf(TO_RADIANS(fovy) / 2.0f));
		dx = ar * dy;
